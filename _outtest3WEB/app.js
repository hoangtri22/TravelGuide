...
function renderTouristSparkline(hourly) {
  ...
  el.innerHTML = data.map((v, i) => {
    const h = Math.max(4, Math.round((v / max) * barH));
  ...
  if (ss) ss.textContent = fmtClock(new Date(now.getTime() - 24 * 60 * 60000));
  if (sm) sm.textContent = fmtClock(new Date(now.getTime() - 12 * 60 * 60000));
  const upd = byId("touristUpdatedAt");
  if (upd) upd.textContent = `Cập nhật lúc ${fmtClock(now)}`;
...
async function loadTouristOverview() {
  ...
  const activated = Number(dash.activatedCount ?? 0);
  const pct = total > 0 ? Math.round((activated / total) * 100) : 0;

  const elOnline = byId("touristStatOnline");
  if (elOnline) elOnline.textContent = String(dash.onlineCount ?? 0);
  ...
  if (elSess) elSess.textContent = String(dash.sessionsToday ?? 0);

  renderTouristSparkline(dash.hourlyActivity);

  const sessList = byId("touristSessionList");
  if (sessList) {
    const live = dash.liveSessions || [];
    sessList.innerHTML = live.length
      ...
      : `<div class="tourist-session-empty hint">Chưa có phiên hợp lệ (refresh token còn hạn).</div>`;
  }
